...
// Listahan ng mga distinct Status na talagang nasa lot_inventory.status
// para sa isang Subdivision — ginagamit para gawing dropdown (sa halip
// na free-text) ang Status filter, iwas typo/mismatch (dating LIKE
// substring match ang gamit kaya sablay ang resulta, hal. tumutugma ang
// "RELEASED" sa "NOT YET RELEASED").
function getSubdivisionMonitorStatuses($body = []) {
    $db = getDB();
    ensureLotInventoryTable($db);
    ensureLotInventoryStatusColumn($db);
    $subd = trim($body['subd'] ?? '');
    if ($subd !== '') {
        $stmt = $db->prepare("SELECT DISTINCT status FROM lot_inventory WHERE sub = ? AND status IS NOT NULL AND TRIM(status) != '' ORDER BY status ASC");
        $stmt->bind_param("s", $subd);
        $stmt->execute();
        $res = $stmt->get_result();
    } else {
        $res = $db->query("SELECT DISTINCT status FROM lot_inventory WHERE status IS NOT NULL AND TRIM(status) != '' ORDER BY status ASC");
    }
    $rows = [];
    while ($r = $res->fetch_assoc()) $rows[] = $r['status'];
    $db->close();
    sendJSON(["statuses" => $rows]);
}
...
function getSubdivisionMonitorLots($body) {
    ...
    $sql = "
        SELECT
            ...
            (
                (SELECT COUNT(*) FROM liaison_records lr3
                    WHERE lr3.lot_inventory_id = li.id AND lr3.or_no IS NOT NULL AND lr3.or_no <> '')
                +
                (SELECT COUNT(*) FROM or_issuance oi3
                    WHERE oi3.lot_inventory_id = li.id AND oi3.or_number IS NOT NULL AND oi3.or_number <> '')
            ) AS or_count,
            ...
            (SELECT d.dt FROM (
                SELECT lr2.or_date AS dt FROM liaison_records lr2
                    WHERE lr2.lot_inventory_id = li.id AND lr2.or_date IS NOT NULL AND lr2.or_date <> ''
                UNION ALL
                SELECT oi2.or_date AS dt FROM or_issuance oi2
                    WHERE oi2.lot_inventory_id = li.id AND oi2.or_date IS NOT NULL AND oi2.or_date <> ''
            ) d ORDER BY d.dt DESC LIMIT 1) AS latest_or_date,
            (SELECT s.val FROM (
                SELECT COALESCE(NULLIF(lr2.or_yr_covered,''), NULLIF(lr2.yr_covered,'')) AS val,
                       lr2.date_saved AS ord_dt
                    FROM liaison_records lr2
                    WHERE lr2.lot_inventory_id = li.id
                UNION ALL
                SELECT COALESCE(NULLIF(oi2.yr, ''), NULLIF(REGEXP_SUBSTR(oi2.or_date, '[0-9]{4}'), '')) AS val,
                       oi2.date_saved AS ord_dt
                    FROM or_issuance oi2
                    WHERE oi2.lot_inventory_id = li.id AND oi2.or_number IS NOT NULL AND oi2.or_number <> ''
            ) s WHERE s.val IS NOT NULL ORDER BY s.ord_dt DESC LIMIT 1) AS rpt_updated
        FROM lot_inventory li
        LEFT JOIN liaison_records lr ON lr.lot_inventory_id = li.id
        WHERE $where
        GROUP BY li.id
...
function searchSubdivisionMonitorLots($body) {
    ...
    if ($status !== '') {
        // Exact match (case/space-insensitive) sa halip na substring LIKE —
        // dati kasi ang "RELEASED" ay tumutugma rin sa "NOT YET RELEASED",
        // "FOR RELEASE", atbp. kaya sablay/sobrang laki ang resulta.
        $where .= " AND UPPER(TRIM(li.status)) = UPPER(TRIM(?))";
        $types .= "s"; $vals[] = $status;
    }
    if ($hasTdRecord === 'yes') {
    ...
    $sql = "
        SELECT
            ...
            tr.municipal AS municipal,
            (
                (SELECT COUNT(*) FROM lot_or_history oh WHERE oh.lot_inventory_id = li.id AND oh.or_number IS NOT NULL AND oh.or_number <> '' AND oh.amount IS NOT NULL AND oh.amount > 0 AND oh.or_number NOT REGEXP '^[0-9]+\\\\.[0-9]{1,2}$')
                +
                (SELECT COUNT(*) FROM liaison_records lr WHERE lr.lot_inventory_id = li.id AND lr.or_no IS NOT NULL AND lr.or_no <> ''
                    ...
                    AND NOT EXISTS (SELECT 1 FROM or_issuance oi4 WHERE oi4.lot_inventory_id = li.id AND UPPER(TRIM(REGEXP_REPLACE(oi4.or_number, '\\\\s*\\\\([^)]*\\\\)\\\\s*$', ''))) = UPPER(TRIM(lr.or_no))))
                +
                (SELECT COUNT(*) FROM or_issuance oi WHERE oi.lot_inventory_id = li.id AND oi.or_number IS NOT NULL AND oi.or_number <> ''
                    AND NOT EXISTS (SELECT 1 FROM lot_or_history oh5 WHERE oh5.lot_inventory_id = li.id AND UPPER(TRIM(REGEXP_REPLACE(oh5.or_number, '\\\\s*\\\\([^)]*\\\\)\\\\s*$', ''))) = UPPER(TRIM(oi.or_number))))
            ) AS or_count,
            COALESCE(
                (SELECT MAX(oh2.yr) FROM lot_or_history oh2 WHERE oh2.lot_inventory_id = li.id AND oh2.or_number IS NOT NULL AND oh2.or_number <> '' AND oh2.amount IS NOT NULL AND oh2.amount > 0 AND oh2.or_number NOT REGEXP '^[0-9]+\\\\.[0-9]{1,2}$'),
                (SELECT MAX(COALESCE(oi2.yr, REGEXP_SUBSTR(oi2.or_date, '[0-9]{4}') + 0)) FROM or_issuance oi2 WHERE oi2.lot_inventory_id = li.id AND oi2.or_number IS NOT NULL AND oi2.or_number <> '')
            ) AS rpt_updated,
            COALESCE(
                ...
                (SELECT oi3.or_date FROM or_issuance oi3 WHERE oi3.lot_inventory_id = li.id AND oi3.or_date IS NOT NULL AND oi3.or_date <> '' ORDER BY oi3.date_saved DESC, oi3.id DESC LIMIT 1)
            ) AS latest_or_date
        FROM lot_inventory li
        LEFT JOIN tax_rates tr ON tr.lot_code = li.sub
        WHERE $where
...
function getSubdivisionMonitorLotDetail($body) {
    ...
    while ($r = $res->fetch_assoc()) {
        ...
        // Laktawan din ang mga row na malinaw na "column-shifted" — nangyayari
        // ito kapag ang orihinal na Excel ay hindi eksaktong 9-column-per-year
        // na layout, kaya sablay ang extraction (gumamit ng FIXED column
        // offset ang lumang extract_lot_data.py sa halip na header-driven na
        // v2 bersyon). Senyales: parang decimal/pera ang laman ng OR Number
        // (hal. "5097.6") sa halip na tunay na OR#/reference format.
        $orNumRaw = trim((string)($r['or_number'] ?? ''));
        $looksShifted = $orNumRaw !== '' && preg_match('/^\d+\.\d{1,2}$/', $orNumRaw);
        if ($looksShifted) continue;
        $orHistory[] = [
            "id"              => (int)$r['id'],
            "lotInventoryId"  => (int)$r['lot_inventory_id'],
            "source"      => "import",
    ...
    // Source 3: or_issuance (OR Issuance module records)
    ensureOrIssuanceTable($db);
    $stmt = $db->prepare("
        SELECT * FROM or_issuance
        WHERE lot_inventory_id = ? AND or_number IS NOT NULL AND or_number <> ''
        ORDER BY date_saved DESC, id DESC
    ");
    $stmt->bind_param("i", $lotInventoryId);
    $stmt->execute();
    $res = $stmt->get_result();
    while ($r = $res->fetch_assoc()) {
        $fromLabel = _orQuarterLabel($r['from_quarter']);
        $toLabel   = _orQuarterLabel($r['to_quarter']);
        $quarterRange = ($fromLabel && $toLabel) ? ($fromLabel === $toLabel ? $fromLabel : "$fromLabel - $toLabel") : ($fromLabel ?: $toLabel);
        $sortYear = !empty($r['yr']) ? (int)$r['yr'] : 0;
        if (!$sortYear && !empty($r['or_date']) && preg_match('/\d{4}/', $r['or_date'], $ym)) $sortYear = (int)$ym[0];
        $yearCovered = !empty($r['yr']) ? ($quarterRange ? $r['yr'] . " (" . $quarterRange . ")" : (string)$r['yr']) : $quarterRange;
        $orHistory[] = [
            "source"      => "or_issuance",
            "orNumber"    => $r['or_number'],
            "date"        => $r['or_date'],
    ...
    // imported CRC data (Source 1) o sa OR Issuance module (Source 3),
    // huwag na ulit isama ang parehong OR mula sa Liaison records (Source 2)
    // — dito nagmumula ang "dobol" na OR entry. Priority ang mas kompleto
    // na record; ang Liaison version na lang ng parehong OR ang tatanggalin.
    $seenOrNumbers = [];
    foreach ($orHistory as $r) {
        if (($r['source'] === 'import' || $r['source'] === 'or_issuance') && !empty($r['orNumber'])) {
...
// Pag-edit ng isang na-import na OR history row (galing sa CSV import,
// table lot_or_history). Ito yung mga row na dating hindi na-eedit kahit
// saan sa system — makikita lang sa OR History modal ng Subdivision
// Monitor pero walang Edit button dati.
function updateLotOrHistory($body) {
    $db = getDB();
    ensureLotInventoryTable($db);

    $id = isset($body['id']) ? (int)$body['id'] : 0;
    if ($id <= 0) sendJSON(["error" => "Invalid record ID."]);

    $yr        = isset($body['yr']) ? (int)$body['yr'] : 0;
    $orNumber  = trim($body['orNumber'] ?? '');
    $amount    = isset($body['amount']) && $body['amount'] !== '' ? (float)$body['amount'] : null;
    $orDate    = trim($body['orDate'] ?? '');
    $fr        = trim($body['fr'] ?? '');
    $to        = trim($body['to'] ?? '');
    $mcLiaison = trim($body['mcLiaison'] ?? '');
    $remarks   = trim($body['remarks'] ?? '');

    if ($yr <= 0) sendJSON(["error" => "Year is required."]);

    $stmt = $db->prepare("
        UPDATE lot_or_history
        SET yr = ?, or_number = ?, amount = ?, or_date = ?, fr = ?, `to` = ?, mc_liaison = ?, remarks = ?
        WHERE id = ?
    ");
    $stmt->bind_param("isdsssssi", $yr, $orNumber, $amount, $orDate, $fr, $to, $mcLiaison, $remarks, $id);
    $ok = $stmt->execute();
    $stmt->close();

    if (!$ok) { $db->close(); sendJSON(["error" => "Failed to update record."]); }

    $db->close();
    sendJSON(["success" => true]);
}

// Pagbura ng isang na-import na OR history row.
function deleteLotOrHistory($body) {
    $db = getDB();
    $id = isset($body['id']) ? (int)$body['id'] : 0;
    if ($id <= 0) sendJSON(["error" => "Invalid record ID."]);

    $stmt = $db->prepare("DELETE FROM lot_or_history WHERE id = ?");
    $stmt->bind_param("i", $id);
    $ok = $stmt->execute();
    $stmt->close();
    $db->close();

    if (!$ok) sendJSON(["error" => "Failed to delete record."]);
    sendJSON(["success" => true]);
}
...
function importSubdivisionMonitorUpdate($body) {
    ...
    foreach ($rows as $row) {
        ...
        if (!isset($seenLotIds[$lotId])) {
            ...
            $av            = ($avRaw === '' || $avRaw === '0') ? null : number_format((float)$avRaw, 2, '.', '');
            $status        = trim($row['lotStatus'] ?? ($row['status'] ?? ''));
            $tdNo          = trim($row['tdNo'] ?? '');
            $tdNoVal       = $tdNo === '' ? null : $tdNo;
            $prevTdNo      = trim($row['previousTdNo'] ?? '');
            $prevTdNoVal   = $prevTdNo === '' ? null : $prevTdNo;

            $updLot->bind_param(
                "sssssssssssssi",
                $buyersName,$buyersName, $lotOwner,$lotOwner, $tctNo,$tctNo, $prevTctNo,$prevTctNo,
                $status,$status, $av, $tdNoVal, $prevTdNoVal,
                $lotId
            );
            $updLot->execute();
            $updated++;
